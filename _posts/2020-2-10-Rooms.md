...
<div class="fold-out-content">Session storage is a way to store information on the client side. The browser will keep a list with keys and values for each website. An alternative is localStorage which you can use in the same way. Local Storage will be stored forever (or until removed by the website) while session storage gets automaticly removed when the user closes the window. <br/><br/>In this case we are only going to use the data directly after we forwared the user to another page. Therefore we can just use session storage.</div>
</div>
