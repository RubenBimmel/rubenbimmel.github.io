...
Let's start by creating our homepage that you see on <a href="http://localhost:3000" target="_blank">http://localhost:3000</a>. There are two options here. Host a new game or join a game as a player. 

The host button opens the game page and does not have to communicate with the server. The join button will try to join a room with the id that the player has entered in the room input field. If the room exists the player will join the game and gets forwarded to the controller page. To do this the homepage needs to check if the room exists on the server. 
